...
void cooperate_scheduler_test1()
{
    ...
    cooperate_task_register(&group1, &task1);
    cooperate_task_register(&group1, &task2);
...
static bool fun5(void *arg)
{
    ...
    static int i = 0;
    i++;
    printf("[cooperate_scheduler Test]: %s:exe succeed times: %d exe fail times: %d cpu_tick %u lpe %u global_resource %d\r\n",
