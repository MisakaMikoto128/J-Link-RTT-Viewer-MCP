...
// #define RS485_4_DIR_PIN LL_GPIO_PIN_10
// #define RS485_5_DIR_PIN LL_GPIO_PIN_8
// #define RS485_4_DIR_Port GPIOA
// #define RS485_5_DIR_Port GPIOA
// #define RS485_4_DIR_Port_CLK_EN() LL_APB2_GRP1_EnableClock(LL_APB2_GRP1_PERIPH_GPIOA)
// #define RS485_5_DIR_Port_CLK_EN() LL_APB2_GRP1_EnableClock(LL_APB2_GRP1_PERIPH_GPIOA)
#define RS485_3_DIR_PIN LL_GPIO_PIN_15
#define RS485_3_DIR_PIN2 LL_GPIO_PIN_3
#define RS485_3_DIR_Port GPIOA
#define RS485_3_DIR_Port2 GPIOB
#define RS485_3_DIR_Port_CLK_EN() LL_APB2_GRP1_EnableClock(LL_APB2_GRP1_PERIPH_GPIOA)
#define RS485_3_DIR_Port2_CLK_EN() LL_APB2_GRP1_EnableClock(LL_APB2_GRP1_PERIPH_GPIOB)
...
void releaseModbus3()
{
    ...
    LL_GPIO_ResetOutputPin(RS485_3_DIR_Port2, RS485_3_DIR_PIN2);
...
void getModbus3()
{
    ...
    LL_GPIO_SetOutputPin(RS485_3_DIR_Port2, RS485_3_DIR_PIN2);
}
...
void gpio_initModbus3()
{
    ...
    GPIO_InitStruct.Pin = RS485_3_DIR_PIN2;
    LL_GPIO_Init(RS485_3_DIR_Port2, &GPIO_InitStruct);
    // 初始化时释放RS485总线
    releaseModbus3();
}
        ...
        // else
        {
            ...
            */
            ...
            if (crc1 != 0)
            {
                ...
                hmodbusRTU->transEvent = MODBUS_RTU_TRANS_ERR_REV_FRME; /* 帧错误 */
                MODBUS_RTU_SET_TRANS_STATE(hmodbusRTU, MODBUS_RTU_TRANS_STATE_WAITING_DISPOSE);
//                ULOG_ERROR("[modbus host] Error: CRC check error!");
                continue;
            }
...
void modbus_rtu_trans_event_inner_deal(ModbusRTUInstance_t *hmodbusRTU)
{
...
#if MOD_BUS_DEBUG == 1
            ULOG_ERROR("[Modbus RTU]: MODBUS RTU TRANS RECEIVE EXCEPTION CODE COM : %d Addr %d\r\n", hmodbusRTU->com, hmodbusRTU->slaveAddr);
#endif // !MOD_BUS_DEBUG
        }
        break;
...
#if MOD_BUS_DEBUG == 1
        ULOG_ERROR("[Modbus RTU]: MODBUS_RTU_TRANS_ERR_TRANS_TIMEOUT COM : %d Addr %d\r\n", hmodbusRTU->com, hmodbusRTU->slaveAddr);
#endif // !MOD_BUS_DEBUG
        break;
    case MODBUS_RTU_TRANS_ERR_REV_FRME:
        ...
        hmodbusRTU->transEvent = MODBUS_RTU_TRANS_RES_NONE;
#if MOD_BUS_DEBUG == 1
        ULOG_ERROR("[Modbus RTU]: MODBUS_RTU_TRANS_ERR_REV_FRME COM : %d Addr %d\r\n", hmodbusRTU->com, hmodbusRTU->slaveAddr);
#endif // !MOD_BUS_DEBUG
        break;
    default:
        break;
...
void modbus_rtu_host_init(ModbusRTUInstance_t *hmodbusRTU, int baud, char parity, int data_bit)
{
    ...
    wordLen = data_bit == 7 ? LL_USART_DATAWIDTH_8B : LL_USART_DATAWIDTH_8B;
    ...
    Uart_SetWriteOverCallback(hmodbusRTU->com, hmodbusRTU->release_bus, NULL);
 ...
 * @param _reg 
 * @param _num 
 * @return uint8_t 0 表示该端口的Modbus RTU主机正在处理请求，忙碌中。1 表示发送成功。
 */
 ...
 * @param Num 返回实际实际读取了多少个uint16_t类型的数据。
 */
int modbus_rtu_get_data_uint32(ModbusRTUInstance_t *hmodbusRTU, uint32_t *pBuf, int Num)
{
    int read_len = 0; // 实际要读取几个指定类型的数据
    // 这里假定了收到的数据是正常的Modbus帧
    read_len = hmodbusRTU->RxBuf[2] / sizeof(uint32_t); /* 数据长度 字节数 */
    read_len = read_len > Num ? Num : read_len;
    uint8_t *data = &hmodbusRTU->RxBuf[3];

    for (size_t i = 0; i < read_len; i++)
    {
        pBuf[i] = ((uint32_t)data[0] << 24) | ((uint32_t)data[1] << 16) | ((uint32_t)data[2] << 8) | ((uint32_t)data[3]);
