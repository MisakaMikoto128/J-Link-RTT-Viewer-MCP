...
{
    unsigned char ret = (keyinpin->GPIOx->IDR & keyinpin->GPIO_Pin) == keyinpin->GPIO_Pin;
    if(keyinpin->pressed_level == KEY_IN_LOW)
    {
        ret = !ret;
    }
    return ret;
...
void KeyGPIOConfig(int readPiNum, int setPinNum, const KeyPin_t *keyinpins, const KeyPin_t *keysetpins)
{
    ...
    for (i = 0; i < readPiNum; i++)
    {
        if(keyinpins[i].pressed_level == KEY_IN_LOW){
...
{
    return (KeyStateValue_t)(keystates % KEY_STATE_NUM);
}
const char *KeyGetEventName(KeyState_t keystate)
{
    KeyStateValue_t event = KeyGetEvent(keystate);
    switch (event)
    {
    case KEY_DOWN:
        return "KEY_DOWN";
...
{
    return (keystate / KEY_STATE_NUM) + 1;
}