...
#include <string.h>
#include "gpio.h"
#define MAX_READ_PIN_NUM 6UL
#define MAX_SET_PIN_NUM 6UL
...
typedef struct
{
    GPIO_TypeDef *GPIOx;
    uint16_t GPIO_Pin;
    KeyInLevel_t pressed_level; // when press, pressed_level
} KeyPin_t;
...
#define KEY_LONG_PRESS_TIME (1000)                              /* unit:ms, hold for KEY_LONG_PRESS_TIME ms and consider it a long press*/
#define KEY_LONG_PRESS_CONTINUE_TIME (KEY_LONG_PRESS_TIME / 7 * 6) /* unit:ms, can't bigger than KEY_LONG_PRESS_TIME*/
#define KEY_DOUBLECLICK_TIME (50)                                  /* unit:ms, press it twice for less than KEY_DOUBLECLICK_TIME ticks(usually ms), it is considered as double - click*/
...
void KeyDriver();
KeyStateValue_t KeyGetEvent(KeyState_t keystates);
const char *KeyGetEventName(KeyState_t keystate);
uint32_t KeyGetKeyID(KeyState_t keystate);

/**
 * KeyID定义为按键的序号，从1开始。
 * KeyEvent定义为按键的事件，包括按下、抬起、长按、双击。
 */
#endif /* KEY_H_ */
