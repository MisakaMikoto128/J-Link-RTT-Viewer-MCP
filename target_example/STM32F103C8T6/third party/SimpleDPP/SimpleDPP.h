...
// SimpleDPP frame max data size
#define SIMPLEDDP_MAX_FRAME_SIZE 256
#define SIMPLEDDP_MAX_FRAME_ENCODED_SIZE (SIMPLEDDP_MAX_FRAME_SIZE * 2 + 2)
...
typedef  size_t (*SimpleDPP_write_t)(const sdp_byte *data, int len);
...
typedef struct SimpleDPP_ {
    ...
    SimpleDPP_write_t SimpleDPP_write;
} SimpleDPP,*pSimpleDPP;


// Externally provided methods
void SimpleDPP_Constructor(SimpleDPP* sdp,sdp_byte *send_buffer,int send_buffer_capacity,sdp_byte *recv_buffer,int recv_buffer_capacity,SimpleDPPRecvCallback_t SimpleDPPRecvCallback,SimpleDPPRevErrorCallback_t SimpleDPPRevErrorCallback,SimpleDPP_write_t SimpleDPP_write);
