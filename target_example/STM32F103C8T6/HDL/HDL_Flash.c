...
/* Interface */
uint32_t HDL_Flash_get_address_by_sector(uint32_t sector);
int HDL_Flash_write_one_sector_nocheck(uint32_t sector, uint8_t *buf);
int HDL_Flash_erase_one_sector(uint32_t sector);
...
int HDL_Flash_write(uint32_t address, uint8_t *data, uint32_t size)
{
    ...
    while (1) {
        HDL_Flash_read(HDL_FLASH_ADDR_OF_SECTOR(sec), (uint8_t *)flash_buffer, HDL_FLASH_SECTOR_SIZE);
        ...
        HDL_Flash_erase_one_sector(sec);
        HDL_Flash_write_one_sector_nocheck(sec, (uint8_t *)flash_buffer);
 ...
 * @return int 返回实际读取到的字节数。
 */
int HDL_Flash_read(uint32_t address, uint8_t *buf, uint32_t size) __attribute__((optnone))
{
...
uint32_t HDL_Flash_get_address_by_sector(uint32_t sector)
{
    ...
    if (sector < HDL_FLASH_SECTOR_NB) {
        addr = HDL_FLASH_START_ADDRESS + sector * HDL_FLASH_SECTOR_SIZE;
    }
    return addr;
}
...
int HDL_Flash_erase_one_sector(uint32_t sector)
{
    ...
    FLASH_PageErase(sector * HDL_FLASH_SECTOR_SIZE);
...
int HDL_Flash_erase_sector(uint32_t sector, uint32_t count)
{
    ...
    for (uint32_t i = sector; i < end_sector; i++) {
        status = HDL_Flash_erase_one_sector(sector);
        if (status != 0) {
            break;
        }
    }
 ...
 *
 * @param sector 将要擦除的第一个扇区的编号,[0 - @ref HDL_FLASH_SECTOR_NB)
 * @param buf 将要写入的数据。大小为 @ref HDL_FLASH_SECTOR_SIZE。
 * @return int 成功返回0，失败返回-1。
 */
int HDL_Flash_write_one_sector_nocheck(uint32_t sector, uint8_t *buf)
{
    ...
    uint32_t flash_addr = HDL_Flash_get_address_by_sector(sector);
...
#if HDL_FLASH_PROGRAMM_BIT_WIDTHT == 16
#define HDL_FLASH_ROGRAM_TYPEP FLASH_TYPEPROGRAM_HALFWORD
#elif HDL_FLASH_PROGRAMM_BIT_WIDTHT == 32
#define HDL_FLASH_ROGRAM_TYPEP FLASH_TYPEPROGRAM_WORD
#elif HDL_FLASH_PROGRAMM_BIT_WIDTHT == 64
#define HDL_FLASH_ROGRAM_TYPEP FLASH_TYPEPROGRAM_DOUBLEWORD
#endif
        status = HAL_FLASH_Program(HDL_FLASH_PROGRAMM_BIT_WIDTHT, flash_addr + idx * sizeof(uint64_t),
                                   data_ptr[idx]);
        if (status != HAL_OK) {
            status = -1;
            break;
